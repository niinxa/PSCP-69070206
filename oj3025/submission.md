...
```text
โจทย์รับinput เลขเดือนและวันที่ ไปตรวจว่าเป็นฤดูไหน

input:
รับเลขเดือนและวันที่เป็นจำนวนเต็ม

output:
ชื่อฤดูการที่ผ่านการตรวจสอบเงื่อนไข

costranints:
เงื่อนไขที่ว่าฤดูการจะเปลี่ยนได้ ถ้าเลขเดือนหาร3ลงตัว และวันที่21ขึ้นไป
```
...
```text
step1: รับค่า
step:2 เช็คเงื่อนไขว่าเลขเดือนอยู่ใน1-3ไหม
step3: ถ้าใช่ก็เช็คต่อว่า month % 3 and day == 21 ไหม
step:4 ถ้าใช่print spring(ฤดูใหม่)
step:5 ถ้าไม่ใช่printฤดูเดิม
step:6-n เช็คแบบนี้ให้ครบทุกฤดู
```
...
```text
ใช้วิธีเหมือนกับแผนแรก แต่มีการเปลี่ยนตรง step 3 เพราะโจทย์ให้วันที่ >= 21
ถึงจะเปลี่ยนฤดูได้
```
...
```text
เช็คว่า เดือนยังหาร3 ไม่ลงตัว แต่วันที่ >= 21 แล้ว ผลลัพธ์จะออกมาเป็นฤดูเดิมไหม
```
...
```text
เช็คว่า เดือนหาร3ลงตัว แต่วันที่ < 21 แล้ว ผลลัพธ์จะออกมาเป็นฤดูเดิมไหม
```
...
```text
เช็คว่า เดือนหาร3ลงตัว แต่วันที่ >= 21 แล้ว ผลลัพธ์จะออกมาเป็นฤดูใหม่ไหม
```
