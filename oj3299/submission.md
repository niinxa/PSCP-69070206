...
```text
โจทย์ให้คำนวณหาว่าช่องสุดท้ายที่ปลูก อยู่ในแถบที่เท่าไหร่ จากการปลูกเป็นแนวทแยง

Input:
จำนวนแถบที่ปลูก ช่องสุดท้ายที่มีการปลูก

Output:
เลขแถบที่ช่องสุดท้ายได้ปลูก

Constraints:
มีการใช้ while เพื่อเช็คจำนวนที่ปลูกไปเรื่อยๆว่าถึงช่องสุดท้ายรึยัง
```
...
```text
Step1: รับค่าจำนวนแถบและช่องสุดท้าย เป็นจำนวนเต็ม
Step2: สร้างตัวแปรnum เก็บ1 ไว้
Step3: สร้างตัวแปร total และ box เก็บจำนวนช่องทั้งหมดและแถบที่อยู่
Step4: ใช้ while เช็ค ว่า total < ช่องสุดท้ายไหม
Step5: ถ้าใช่ ให้ total += num และ num += 1
Step6: ถ้าไม่เข้าเงื่นไขแล้ว ให้คิดค่า box จาก (num - 1 + n - 1) // n
Step7: พิมพ์ box
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก ฉันใช้ whlie เพื่อวนลูปเช็คไปเรื่อยๆว่า total < ช่องสุดท้ายไหม ละให้ total เพิ่มทีละnum โดยที่num เพิ่มขึ้นทีละ1เหมือนกัน (num คือจำนวนสามิกในแต่ละการทแยง) ก้จะได้จำนวนกระถางทั้งหมดมา จากนั้นพอลูปจบ ก็ไปหาว่าอยู่แถบที่เท่าไหร่โดยใช้สูตรนี้ box = (num - 1 + n - 1) // n ที่เอา num - 1 เพราะ num นับเกิน และ n-1 เพื่อใ้หารแล้วปัดขึ้น
```
...
```text
นับทีละ 1 แถบ สามารถอ่านการคำนวณได้ง่าย
```
...
```text
นับทีละ 3 แถบ ทำให้มีความซับซ้อนตอนคำนวณ
```
...
```text
นับทีละ5 แต่มีแค่หนึ่งช่องที่ต้องปลูก ผลเลยออกมาเป็น1
```
