...
```text
หาจำนวนเฉพาะตามช่วง ที่รับเข้ามา

Input:
จำนวนเริ่มต้น และ จำนวนตัวสุดท้าย ในบรรทัดเดียวกัน

Output:
จำนวนเฉพาะแต่ละตัว ในช่วงนั้น
จำนวนของจำนวนเฉพาะ

Constraints:
จำนวนเฉพาะต้องหาร 1 และตัวมันเอง ลงตัว
```
...
```text
step1: ฉันใช้ map(int,input().split()) เพื่อแยกตัวเริ่มต้น และตัวสุดท้าย
step2: ฉันสร้างตัวแปร count เก็บจำนวนทีหารลงตัว และ list ไว้เก็บจำนวนเฉพาะ
step3: ฉันใช้ for loop ให้ i วนตัวแรก - ตัวสุดท้าย + 1
step4: ฉันใช้ for loop ให้ j 1 - ตัวสุดท้าย + 1 วนข้างใน for loopแรก 
step5: เช็คว่า i % j ลงตัวไหม
step6: ถ้าลงตัว ให้ count += 1
step7: ใช้ if นอก loop 2 เพื่อเช็คว่าถัา count == 2
step8: ให้เพิ่มเลขตัวที่ i ลงใน list
step9: รีให้ count == 0
step10: ใช้ if นอกทุก loop เพื่อเช็คว่าใน list มีเลขไหม
step11: ถ้ามีให้พิมพ์ เลขทั้งหมดใน list โดยใช้  " ".join(map(str,prime))
step12 : พิมพ์จำนวนของจำนวนเฉพาะทั้งหมด
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก  ฉันใช้ for loop แรก เพื่อเอาเลขในช่วงที่ต้องการหาจำนวนเฉพาะ ส่วน for loop อีกอันที่อยู่ข้างใน ฉันเอาไว้หาเลขตั้งแต่1 - ตัวสุดท้าย จากนั้นก็จะให้เลขในช่วง % เลขตั้งแต่1 - ตัวสุดท้าย เพื่อหาว่าตัวไหนเป็นจำนวนเฉพาะบ้าง 
```
...
```text
เช็คว่าโค้ดเราสามารถหาจำนวนเฉพาะได้ไหม
```
...
```text
เช็คว่าถ้าเป็นเลขที่ไม่ใช่จำนวนเฉพาะ จะพิมพ์จำนวนของจำนวนเฉพาะเป็น 0 ไหม
```
...
```text
เพื่อหาจำนวนเฉพาะทั้งหมดของช่วงนี้ ว่าตรงทั้งหมดไหม
```
