...
```text
โจทย์นี้ให้พิมพ์ * ล้อมรอบข้อความ5อัน โดยกรอบจะอิงตามขนาดข้อความที่ยาวที่สุด

Input:
โปรแกรมรับข้อความทั้งหมด5ข้อความ

Output:
ข้อความ5ข้อความ ที่ล้อมรอบด้วย *

Constraints:
ขนาดของ * ที่ล้อมรอบหัวท้าย ==  ขนาดของข้อความที่ยาวที่สุด + 4
และกรอบข้างๆ คำนวนได้จากขนาดข้อความที่ยาวที่สุด
```
...
```text
step1:  สร้างlist มาเก็บข้อความ และ สร้างตัวแปร num ไว้เก็บค่าข้อความที่มากที่สุด
step2: ใช้ for loop วน 5 ครั้งเพื่อรับข้อความที่ใช้.rstrip() เพื่อตัดช่องว่างหลังข้อความออกให้หมด
step3: เพิ่มข้อความที่รับมาเข้าไปในlist
step4: ใช้ if เช็คว่า num < ขนาดข้อความไหม
step5: ถ้าใช่ ให้ num == ขนาดข้อความนั้น
step6: เริ่มทำกรอบแถวบนสุด โดยสร้างตัวแปร x เก็บ "*" * num + 4
step7: print(x)
step8: ใช้  for loop วนข้อความใน list
step9: ถ้าขนาดข้อความนั้น == num 
step10: ให้ปริ้น x[0], ข้อความ, x[num + 2]
step11: ถ้าไม่ใช่ ให้คำนวนหาช่องว่างที่ต้งเว้นก่อนใส่ *
step12: สร้างตัวแปร diff เก็บค่าช่องว่างที่คำนวณจาก num - len(ข้อความนั้น)
step13: print x[0], ข้อความ + " " * diff,  x[num + 2]
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก ใช้ listเก็บข้อความและสร้างตัวแปรมาเก็บขนาดข้อความที่มากที่สุด เพื่อเอาไปคำนวณกรอบ ใช้ string slicing เพื่อเอา * ให้ตรงตำแหน่งของขอบกรอบ
```
...
```text
เพราะคำว่า Hello World มีช่องว่างตามหลังอยู่ แต่กรอบควรครอบแค่หลังตัวสุดท้าย เว้น1ช่อง
```
...
```text
บรรทัดที่4 มีแค่ช่องว่าง ก็ควรเว้นไว้ด้วย
```
...
```text
กรณีปกติ ไม่มีช่องว่างเกิน หรือ ขาด ขนาดตัวอักษรเท่ากันทุกตัว
```
